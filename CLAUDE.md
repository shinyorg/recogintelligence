...
**Detection is separate and lives in the Sample.** The library never sees the camera. `Shiny.Maui.Controls.Camera.Face` raises `DetectionCaptured(photo, FacesDetectedEventArgs)`; the Sample (`FaceDetectionExtensions.cs`) picks the `Largest()` `DetectedFace` and maps its image-space `Bounds` to a Core `FaceBox` (pixel coordinates). The library's contract is "give me the photo bytes + a `FaceBox`."
...
- **Coordinate space**: `DetectedFace.Bounds` and the captured `CameraPhoto` are paired by `DetectionCaptured`; verify on-device that bounds map 1:1 onto the still (front-camera mirroring/rotation). If not, scale the box by photo/preview dimensions before cropping.
