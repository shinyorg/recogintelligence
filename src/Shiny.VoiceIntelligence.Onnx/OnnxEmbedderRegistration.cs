...
{
    /// <summary>
    /// Use the ONNX speaker embedder. Works with both raw-waveform models (classic ECAPA/x-vector) and
    /// feature-input models (WeSpeaker / sherpa-onnx CAM++ exports, input <c>feats [batch, frames, 80]</c>) —
    /// the format is detected from the model itself, or forced via <see cref="OnnxEmbedderOptions.InputMode"/>.
    /// Note <see cref="OnnxEmbedderOptions.Dimensions"/> must match the model's real output width (512 for
    /// CAM++/WeSpeaker, 192 for many ECAPA exports); it sizes the vector store before the model loads, so a
    /// mismatch throws on the first embed. Configure the model via
        ...
        Action<OnnxEmbedderOptions> configure)
    {
        ...
        return builder.UseEmbedder(_ =>
        {
            ...
            if (options.ModelBytesProvider is { } provider)
                return new OnnxEcapaEmbedder(() => CreateSession(provider()), options.Dimensions, options.SampleRate, options.InputMode);
            if (options.ModelBytes is { } bytes)
                return new OnnxEcapaEmbedder(bytes, options.Dimensions, options.SampleRate, options.InputMode);
            if (!string.IsNullOrWhiteSpace(options.ModelPath))
                return new OnnxEcapaEmbedder(options.ModelPath!, options.Dimensions, options.SampleRate, options.InputMode);
